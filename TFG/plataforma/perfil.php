...
$rol = $_SESSION['tipo_usuario'] ?? 'alumno';

// 1. ASIGNACIÓN INTELIGENTE DE TABLAS
if ($rol === 'profesor') {
    $tabla = 'Profesores';
} elseif ($rol === 'admin') {
    $tabla = 'Administradores'; // Intenta buscar en una tabla de administradores
} else {
    $tabla = 'Alumnos';
}
...
// 2. OBTENER DATOS DEL USUARIO (BLINDADO ANTI-ERRORES)
$user = [];
$res = $db->query("SELECT * FROM $tabla WHERE id = ?", [$id_usuario]);

if (is_array($res) && !empty($res)) {
    // Si te encuentra en la base de datos, carga tus datos
    $user = $res[0];
} else {
    // FALLBACK: Si eres un Admin sin tabla, crea un perfil virtual para no romper la web
    $user = [
        'id' => $id_usuario,
        'nombre' => 'Administrador',
        'apellidos' => 'del Sistema',
        'email' => 'admin@academiaterra.com',
        'foto' => '',
        'password' => '',
        'curso_asignado' => 'Panel de Control (Acceso Total)'
    ];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Solo permitimos modificar si realmente existe en la BD (evita errores con el perfil virtual)
    if (is_array($res) && !empty($res)) {
        ...
        $error = "Como Administrador del sistema, tus datos están protegidos y no se pueden modificar desde aquí.";
    }
}

// Generación de avatar segura
$foto_display = !empty($user['foto']) ? "../" . $user['foto'] : "https://ui-avatars.com/api/?name=".urlencode($user['nombre'] ?? 'Admin')."&background=0891b2&color=fff&size=200&length=1";
?>
<!DOCTYPE html>
<html lang="es">
<head>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 text-center shadow-xl h-fit">
                <div class="relative inline-block">
                    <img src="<?php echo htmlspecialchars($foto_display); ?>" class="w-40 h-40 rounded-full object-cover border-4 border-zinc-800 shadow-2xl">
                ...
                <h2 class="text-xl font-bold text-white mt-6"><?php echo htmlspecialchars($user['nombre'] ?? 'Usuario'); ?></h2>
                <div class="mt-2">
                ...
                <p class="text-zinc-500 text-sm mt-4"><?php echo htmlspecialchars($user['email'] ?? ''); ?></p>
            </div>
                ...
                <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-xl">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-white mt-1"><?php echo htmlspecialchars(($user['nombre'] ?? '') . " " . ($user['apellidos'] ?? '')); ?></p>
                        </div>
                        <div>
                            <label class="text-xs text-zinc-500 uppercase font-bold">Curso / Ciclo</label>
                            <p class="text-white mt-1"><?php echo htmlspecialchars($user['curso_matriculado'] ?? $user['curso_asignado'] ?? 'N/A'); ?></p>
                        </div>
                    </div>
                </div>
